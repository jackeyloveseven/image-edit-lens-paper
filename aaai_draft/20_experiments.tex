...
for the time-lens commit sweep and the preview lever, 20 for the
...
The decision is present far earlier than the head can read it, held in
a basis the head cannot parse until the last few blocks. Decided early,
read out late.
...
dimension). Readability, necessity, and writability thus dissociate
three distinct ways.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.9\columnwidth]{figs/fig6_causality/fig6_causality.pdf}
...
\emph{Committed} (content in the internal image) and \emph{separated}
...
crossover, the cross-seed probe, and layer-band ablation on the two
further checkpoints from \emph{Setup} (FireRed-Image-Edit-1.1 and
Rapid-AIO). \textbf{The translation boundary and the early decision
replicate.}
...
architecture, so the readable$\neq$necessary result above holds
...
layer 53, classifier-free guidance has no architectural reason to need
...
cross-model checks above cover two checkpoints of the same lineage on
a subset of measurements; and we claim nothing about ``intent'' or
about recovering the network's ``true'' reasoning. The semantic
readout inherits CLIP's failure modes in identifiable ways -- pure
sampling noise can score as beach at $0.795$, while an obviously-red
recolored car \emph{under}-scores ($0.09$--$0.16$) against a
\emph{failed} flat patch ($0.70$--$0.84$) -- which is why every
quantitative claim is paired with a pre-registered visual verdict, and
the image wins ties. Four of our own initial misreadings are retained
as data in the supplement (a thumbnail ``revision'' erased by
quantification, a continuous novelty law that did not survive, a
steering ``clean flip'' that was really a flat patch, and the
over-strong conclusion drawn from it, which a silhouette mask
refuted). We claim only that, on \model{}, the
readable/necessary/writable dissociations we report are real and
