...
\begin{figure*}[t]
  ...
  \textbf{A}: four decodes of \emph{car\_red} at $t{=}4$; the image-like
  and standard conventions reverse between L24 and L59. \textbf{B}: 3-way
  probe accuracy (chance $0.33$); the outlined L6/$t$4 cell reaches $0.883$
  while the frozen head at the same state still reads noise. Red score
  pills in \textbf{A} mark target probability above $0.5$.}
  \label{fig:mechanism}
\end{figure*}
...
\begin{figure}[t]
  ...
  tuned lens (fit on non-color edits, zero-shot on \emph{car\_red}) shows
  the edit entering a coherent internal image at $t{=}0$: L24 retains the
  source-blue car, L36 turns it orange-red, and L52 resolves the edited
  scene. Because the sampled pixels are pure noise at this step, the image
  comes from the decoded internal code. \textbf{B}: on the identical
  L36/$t{=}12$ state, the frozen head produces a cyan-car anti-image while
  the tuned lens recovers the coherent red-car scene; the final output is
  shown for reference.}
  \label{fig:tunedlens}
\end{figure}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.93\textwidth]{figs/fig10_boundary/fig10_boundary.pdf}
  \caption{\textbf{Translation band L52--54.} Dense L44--L59 sweep on
  \emph{car\_red}. \textbf{A}: standard vs.\ direct-$v$ CLIP scores cross in
  the same band at $t{=}4$ and $t{=}16$ (midpoints 52.7 and 53.6).
  \textbf{B}: $\cos(v_\ell,v_{59})$ rises sharply in that neighborhood.
  \textbf{C}: probe accuracy stays flat through L58 and drops at L59 only,
  separating translation from the terminal loss of linear readability.}
  \label{fig:boundary}
\end{figure*}
