...
\Large Predictable Early, Translated Late: A Layer--Time Lens for Diffusion Image Editors}
...
\twocolumn[
  ...
  two-axis law, the mask-recomposition carrier constraint and the
...
the velocity code near L52--54: the crossover midpoint is stable within
one layer across the two densely measured timesteps, while transition
width varies from one to four layers. The boundary survives a same-lineage
...
carry obeys a tested \emph{carrier constraint}: attributes are writable wherever a
...
forward passes with all four edit families intact; zero-CFG failures are
consistent with a role in scope resolution (\S\ref{sec:cfgfreelunch}); the layer--time
...
final image quality. We adopt the fixed-prompt outcome protocol and held-out selection structure,
...
Breaking the Lock-in~\citep{kwon2026lockin} is the closest temporal neighbor:
...
\citep{prakash2026dreamreader} is the closest tooling neighbor, unifying
representation extraction, patching, steering, and stitching for text-to-image
models. Our scope is narrower: a frozen-head/tuned layer--time lens for a joint
...
off-distribution steering~\citep{cassano2026look}, and that activation
patching with multiple mediators can hide substantial interaction effects
~\citep{vaidyanathan2026curse}. Activation
...
our headline findings, including the carrier constraint
...
\textbf{A dense boundary sweep separates stable location from variable
sharpness.} The four-variant crossover and the L59 probe dip
...
\textbf{The translator transfers across the tested held-out edits.}
Trained with zero color edits, it renders the held-out color edit
coherently at every tested depth. For readout purposes, the goal-to-flow
translation near L52--54 is therefore well approximated by a
per-(layer, timestep) affine change of basis across this battery, rather
than being specific to \emph{car\_red}.
...
statistics of $D$ against a final-output difference mask $R$ (the
...
between $D_{\text{L52},t2}$'s own top-10\% mass and the final-output
difference mask $R$ is $0.33$ across the 10 cases -- $6.6\times$ the
...
mechanism is the reference-mask definition itself: $R$ is fixed at the
...
where the culprit is not CLIP but the mask used as a geometric reference
...
\begin{figure}[t]
  ...
  Right: $D_{\text{L52},t2}$ (grayscale) with the final-output difference
  mask $R$'s boundary overlaid in red. \emph{remove\_duckfloat}: the
  ...
  reference mask $R$ is padded roughly $10\times$ by collateral fur
...
\citet{cassano2026look}. It does not make whole-token replacement a valid necessity test, nor
