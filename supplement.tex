...
\usepackage[a4paper,top=2.2cm,bottom=2.4cm,left=1.8cm,right=1.8cm,columnsep=0.65cm]{geometry}
...
\Large When Is the Edit Decided? A Layer--Time Lens on How a Diffusion Editor Thinks with an Internal Image}
...
\twocolumn[
  ...
  \small
  \textbf{Abstract.}
  How does an instruction-following image editor arrive at its edit?
  The intuitive picture -- the edit developing gradually, sharpening
  layer by layer and step by step like a photograph in developer fluid
  -- turns out to be wrong in a specific, measurable way. We build the
  first logit-lens analogue for a diffusion-transformer (DiT) editor,
  \model{} (60 blocks), by decoding intermediate residual streams
  through the model's own frozen output head, and pair every passive
  reading with linear probes and causal interventions (ablation,
  transplant, steering). What emerges is an editor that \emph{thinks
  with an internal image}: the edit is \emph{decided early},
  \emph{bound to a spatial carrier} that determines whether it can be
  rendered cleanly, and \emph{translated late} into the flow field that
  actually paints it. A per-(layer, timestep) tuned lens renders this
  internal image directly at every depth, letting us watch the edit
  enter it layer by layer at the very first denoising step. Decided
  early: the edit is linearly readable by
  layer 6 (cross-seed probe 0.883 where the frozen head reads only
  0.005--0.14), the instruction is load-bearing in the first 11 blocks
  and functionally complete by block 36, and across a 20-case battery
  onset is delayed only for the 2 edits that insert genuinely novel
  structure -- no continuous novelty proxy predicts it ($\rho=0.24$,
  $p=0.30$). Translated late: most of the stack carries a
  linearly-readable \emph{target-image code} that a sharp,
  timestep-invariant band (layers 52--54, transition width as narrow
  as 1 layer, confirmed by a four-variant decoding test) converts into
  a \emph{velocity code}. This boundary is edit-family universal and
  survives both an independently fine-tuned checkpoint and a 4-step
  distilled merge (cross-model probe accuracy 0.84--0.92, direction
  cosine 0.83, and -- on the fine-tune -- the full tuned/differential
  lens toolchain itself, deep-layer $R^2$ 0.97--0.999). Decision
  \emph{depth}, however, is not universal: appearance edits are
  readable by layers 6--12, while structure edits crystallize only
  around layer 36. Bound to a carrier: a probe
  direction carries geometry-free semantics -- a hue, a style,
  ``nothing here'' -- but not \emph{where to stop}. Steered into a
  bounding box it overwrites geometry; confined to the edited object's
  own silhouette, the same direction turns every previously-failing
  dose into a perfect in-place recolor. Conjuring an object outruns any
  single direction: a rank ladder over the donor$-$recipient difference
  instead resolves object identity into coarse-to-fine linear shells --
  category at rank 1, a coherent instance by rank 4, material by 16,
  donor-faithful by 64 ($2\%$ of the model dimension) -- with the probe
  direction itself inert. The read and write subspaces, in other words,
  are not the same. Readability, causal necessity, and linear
  writability thus dissociate three different ways, and we map exactly
  where. Because these
  readings are causal, they convert into training-free levers:
  direction-plus-mask gives donor-free, prompt-free in-place attribute
  editing with a dose dial; truncating classifier-free guidance after
  step 2 saves 45\% of forward passes with all four edit families
  visually intact (CFG's real role is scope resolution, not deciding
  the edit); the decision structure survives step distillation at
  $7.7\times$ speed (20/20 cases); and a probe-derived saliency map
  (99\% vs.\ 38\% in-region mass) predicts step budgets and rescues an
  under-strength edit by mid-stack transplant (0.619$\to$0.819). A
  differential form of the same tuned lens (edit run minus a matched
  ``keep it unchanged'' run) turns the internal edit plan into a
  directly measurable quantity -- content commits to it early while the
  differential itself separates into the edit region only gradually --
  and, because the no-op run does not depend on the edit prompt, the
  same differential already previews where a candidate edit will land
  after just 15\% of the denoising trajectory. Every
  intervention was pre-registered and every verdict made on the images
  themselves; four of our own initial misreadings are retained in the
  text as data. This is exploratory, single-model-family work, and we
  are explicit throughout about where the lens is unreliable.
  \vspace{1.0em}
  \end{minipage}\end{center}
  \begin{center}\begin{minipage}{0.92\textwidth}
  \small
  \textbf{Preface.} This document is the extended version accompanying
  our anonymous 7-page AAAI submission of the same title, and is
  provided as supplementary material for review. The main paper
  establishes the spine of the argument: the edit is decided early and
  translated late into the flow field in a sharp, timestep-invariant
  band at layers 52--54; readability, causal necessity, and linear
  writability dissociate in three distinct ways that we localize
  precisely; the decision structure generalizes across an independently
  fine-tuned checkpoint and a distilled merge; and a mechanism-derived
  classifier-free-guidance truncation lever falls out as a practical
  consequence. This supplement additionally provides, in full: the
...
Between reading ``turn the car red'' and painting it, what does an
image editor actually do? The mechanics of diffusion suggest a
gradual-development story: sixty transformer blocks times twenty
denoising steps, each contributing its small share, the edit surfacing
progressively like a photograph in developer fluid. This paper is
about measuring that story and finding it wrong. In the model we
study, an edit is not developed -- it is \emph{decided early}
(linearly present within the first handful of blocks), \emph{bound to
a spatial carrier} that determines whether it can be rendered cleanly,
and only \emph{translated late}, in the last few blocks, from an
internal goal-image code into the flow field that does the painting.
Nearly everything else we report follows from taking this one sentence
seriously and testing each of its clauses causally.
...
We build a layer--time lens for a production DiT image editor,
\model{}, and use it to ask three concrete questions about how an
instruction-following edit (\eg{} "turn the car red") is actually carried
out inside the network. \textbf{When} in the 20-step denoising trajectory
does the edit commit -- immediately, or does the model visibly keep
"thinking"? \textbf{Where} in the 60-block depth does the edit become
legible, and is legibility the same thing as causal importance?  And,
once we have candidate answers from passively reading activations,
\textbf{does intervening on those activations confirm them} -- does
ablating or transplanting the hidden states the lens points to actually
change the output the way the lens predicts?
...
network's \emph{output-space} code; the decision is present, just
represented in a different basis until the last few blocks translate it.
Causal probes (ablation, hidden-state transplant across runs) are needed
to tell whether "readable" activations are also "necessary" ones -- in
one of our two running examples they are, and in the other they are not.
...
We find the intermediate image already present \emph{latently} inside a
generative editor: a target-image code formed within the first few
blocks, readable by a linear probe, steerable, transplantable across
runs -- and translated into pixels only at the end. The lens also
...
\paragraph{Contributions.} Three, in increasing order of practical
consequence. \textbf{(1) An instrument, with a discipline.} The first
...
mechanism: the editor thinks with an internal image.} The edit is
decided early -- linearly readable by layer 6, the instruction
load-bearing in the first 11 blocks and complete by 36 -- and carried
as a target-image code for most of the depth. It is translated into
the velocity code only in a sharp, timestep-invariant band (L52--54)
that is edit-family universal and survives fine-tuning and 4-step
distillation, while decision depth splits by family (appearance
shallow, structure mid-stack). A per-(layer, timestep) tuned lens
...
dimension). Readability, causal necessity, and linear writability
dissociate three different ways, and we map where
(\S\ref{sec:mechanism}--\S\ref{sec:recompose}). \textbf{(3) Levers:
mechanism converts to practice.} Because the readings are causal, they
become training-free tools: probe direction $+$ silhouette mask gives
...
job as scope resolution (\S\ref{sec:cfgfreelunch}); the decision
...
previewable quantity -- content commits to it early but the
...
can find one before the network's own head is willing to commit to it.
...
mid-stack target-image code formed long before any pixel is committed,
...
confounded metrics) that motivate our paired ablate/transplant design
and our insistence on reporting both "readable" and "necessary" as
separate, possibly conflicting, measurements. Activation
...
into a semantic score in \S\ref{sec:readout}. Our lens is architecture-
and scale-agnostic in principle but has only been validated on one
model, \model{}~\citep{qwen2025image}, whose transformer backbone follows
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{figs/fig2_method.pdf}
  \caption{Method overview. The image stream concatenates noisy target
...
per requested step); it is the axis along which we ask \emph{when} an
edit commits (\S\ref{sec:when}).
...
\subsection{Causal probes: ablation and transplant}\label{sec:interventions}
...
\textbf{Ablate.} Zero (or mean-replace, using the mean of the
...
the ablation. An empty ablation spec is verified to reproduce the
baseline output bit-for-bit, which we use as a sanity check that the
intervention machinery introduces no side effects of its own.
...
donor layer band is causally sufficient for that edit, independent of
whether ablating the same band in the donor's own run would have been
\emph{necessary}.
...
Both interventions are applied identically and independently to the
...
lens, and (c) region-restricted 3-class probes to obtain decision
...
non-preserving background replacement. The subsections establish the
opening sentence clause by clause: \emph{decided early}
(\S\ref{sec:when}, \S\ref{sec:mechanism}--\S\ref{sec:injection}),
\emph{translated late} (\S\ref{sec:depth}, \S\ref{sec:mechanism},
\S\ref{sec:families}), \emph{bound to a spatial carrier}
(\S\ref{sec:causal}--\S\ref{sec:steer}, \S\ref{sec:recompose}), and
what the mechanism buys in practice (\S\ref{sec:applications},
\S\ref{sec:cfgfreelunch}).

\subsection{When: edit commit time is edit-dependent}\label{sec:when}

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{figs/fig3_commit_curves.pdf}
...
time: if the edit surfaces progressively, the commit step should be
late and roughly uniform across edits. It is neither.
...
commit step is not a fixed property of the sampler -- it depends on what
the edit asks for. Table~\ref{tab:battery} reports, for
each case, the step at which the time-lens $P(\text{target})$ first
reaches within 0.05 of its step-19 value (\texttt{commit\_step}) and the
...
\bottomrule
\end{tabular}
\caption{Commit step and earliest head-readable layer, all 10 battery
cases (data: \texttt{runs/a5\_aggregate.json}). $^\dagger$Object-removal
CLIP readouts are unreliable (\S\ref{sec:limits}); the commit/first-layer
...
texture edits commit at step 0, edits that introduce a new region of
...
the same edits that commit late in time also require the deepest
first-readable layer.
\subsection{Where in depth: late materialization, inverted mid-stack}\label{sec:depth}

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{figs/fig4_depth.pdf}
...
\begin{figure}[t]
  ...
  \caption{Left: four ways of turning an intermediate layer's frozen-head
...
Here the opening sentence earns its \emph{decided early} and
\emph{translated late} clauses at once. The depth-lens snap in
...
\textbf{Cross-seed linear probe.} A frozen head is one specific,
non-trainable projection; a linear probe fit directly on the captured
hidden states is a different, more permissive question -- is the
decision linearly present at all, in \emph{some} basis, whether or not
the network's own head can read it. We fit a 3-way probe (predicting
which of three colors was requested) on hidden states from one seed and
evaluate on a held-out seed, at every layer/step cell of an
$11\times5$ grid (Figure~\ref{fig:mechanism}, right; chance $=0.333$).
The result is the paper's sharpest readable/necessary dissociation
signal: at L6, $t{=}4$ -- five blocks into the network, at an early
denoising step -- cross-seed probe accuracy is already $0.883$, while
the frozen-head lens at the identical cell reads
$P(\text{red car}) = 0.111$ (\texttt{a1\_p\_red\_same\_cells},
\S\ref{sec:depthlens}). The earliest layer/step cell to cross 0.9
accuracy is L36/$t$0 ($0.906$); by contrast, accuracy at the terminal
layer L59 is consistently \emph{lower} than at L48--L54 (\eg{}
$0.70$--$0.75$ across steps, vs.\ $0.80$--$0.89$ one to two "layer rows"
earlier) -- the same last-block translation that resolves the frozen
head's sign convention appears to cost the representation some of its
linear separability, plausibly because the terminal layer is now encoding
a velocity optimized for the sampler's next step, not for a
color-classification probe.
...
between the shallow layer where a probe first reads the decision and
...
L44, and the car fully red by L52: the ``decided early'' clause of
\S\ref{sec:intro}, rendered directly rather than inferred from a probe
score.
...
color was already fully committed -- the internal image shows an
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{figs/fig13_transfer_onset.jpg}
...
\subsection{Differential lens: commitment is early, separation is
...
\textbf{Commitment is early; separation of the differential is
...
-- red eyes, a black trunk -- is already present at $t{=}0$: commitment
is early, exactly as \S\ref{sec:when}--\S\ref{sec:tunedlens} report.
...
the edit's spatial footprint only gradually. Commitment (is the content
in the internal image yet) and separation (has the differential
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.98\textwidth]{figs/fig14_diff_lens.jpg}
...
top row is already committed at $t{=}0$, and the bottom row is not yet
...
The depth lens and the cross-seed probe both locate a color \emph{decision}
by layer 6; neither tells us when the text-conditioned \emph{instruction}
...
up with \S\ref{sec:mechanism}'s probe result -- if the color decision is
...
finding that the decision is linearly readable by layer 6: the instruction
must already be fused into the image stream within the first 11 blocks
for a linear color decision to exist that early, and Table~\ref{tab:injection}
...
\subsection{Causal localization: necessity}\label{sec:causal}
...
The depth lens (\S\ref{sec:depth}) and the cross-seed probe
(\S\ref{sec:mechanism}) both point to early-to-mid layers as where the
edit decision is linearly present. We now ask whether those layers are
also causally \emph{necessary}, using layer-band and step-window
ablation (\S\ref{sec:interventions}; zeroing is applied to target-grid
tokens, both CFG passes, all 20 steps unless a step window is also
specified) on our two running examples.

\textbf{Layer bands.} For \emph{car\_red}, ablating layers 0--11 is
catastrophic ($P(\text{red car})$ collapses to $0.049$) while ablating
layers 48--59 is nearly harmless ($0.826$, close to the unablated
baseline). For \emph{beach\_background} the profile is a near-mirror
image: ablating 0--11 only partially hurts ($0.193$) while ablating
48--59 is now the catastrophic band ($0.040$; Figure~\ref{fig:causal}A).
The edit that commits early in time (\S\ref{sec:when}) also depends on
early layers; the edit that commits late in time depends on late layers.

\textbf{Step windows.} Ablating a 4--6 step window at 4 mid layers
($\ell\in\{12,24,36,48\}$) barely dents \emph{car\_red} regardless of
\emph{which} window is chosen ($0.92$--$0.96$ across all four
non-overlapping windows) -- consistent with the color decision being
redundantly reconstructible from the always-clean condition tokens at
whichever steps remain unablated. \emph{beach\_background} again
...
window ($0.627$ car, $0.134$ beach), showing the redundancy is
partial, not absolute (Figure~\ref{fig:causal}B).
...
\emph{every} layer barely dents the color decision ($P(\text{red
...
fidelity than cutting only the late half (layers 30--59): the source
image's grounding of everything the edit does \emph{not} touch is an
early-layer phenomenon, even though (per the layer-band result above)
the edit \emph{itself} is also decided early.
...
\label{tab:condtok}
\end{table}
\subsection{Transplant: sufficiency without necessity}\label{sec:swap}

Necessity (\S\ref{sec:causal}) is only half the causal story. We take a
\emph{car\_green} recipient run and, at each of four candidate donor
...
$0.940$ for \textbf{L48--59} (Figure~\ref{fig:causal}C) -- the exact
band that \S\ref{sec:causal} showed is causally \emph{dispensable} for
this same edit ($0.826$ success survives ablating it entirely). The
donor's late-layer representation is sufficient to install the edit
when it is present, even though the recipient's own late-layer
computation is not necessary for the recipient to arrive at the edit on
its own. This is the paper's central dissociation, stated plainly:
\emph{readable, sufficient, and necessary are three different
properties, and a layer band can have any two without the third.} A
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{figs/fig9_steering.pdf}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.98\textwidth]{figs/fig7_applications.pdf}
...
\textbf{Decision saliency.} A linear probe trained on the captured
...
\textbf{Step-budget prediction.} \S\ref{sec:when} showed edits commit at
...
reference. The two early-committing cases are already within $0.02$ of
their reference score at 4 steps (color\_car: $0.938$ vs.\ $0.929$;
style\_watercolor: $0.994$ vs.\ $0.997$); \emph{add\_boat}, whose
commit step is latest in our battery, is slightly behind at 4 steps
...
the edit decision is actually legible, magnitude alone is actively
misleading.
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.98\textwidth]{figs/fig11_replication.pdf}
...
Every finding so far concerns one checkpoint, \model{}. We ask whether the
circuit of \S\ref{sec:mechanism}--\S\ref{sec:injection} and its causal
structure (\S\ref{sec:causal}) are wired into the shared transformer
architecture -- and would be expected on any checkpoint built on it -- or
are specific to this one model's training, by re-running the depth-lens
...
\textbf{The boundary and the readable-decision layer replicate.} FireRed's
...
shape, from an independent fine-tune. Rapid-AIO's color edit commits
immediately in its compressed 4-step schedule ($P(\text{red})@\text{L59}
= 0.955$ at $t{=}0$, staying flat and high through $t{=}3$), the same
immediate-commit pattern \S\ref{sec:when} found for \model{}'s 20-step
color edits.

\textbf{The decision direction itself transfers.} A probe trained on one
...
have cosine similarity $0.833$. The color-edit decision appears to live in
...
\textbf{One genuine divergence.} Layer-band ablation of \emph{car\_red}
replicates the early-layer necessity of \S\ref{sec:causal} almost exactly
-- L0--11 is near-catastrophic for both ($0.049$ \model{}, $0.082$
FireRed) -- but diverges sharply in the late band: ablating L48--59 leaves
\model{}'s edit essentially intact ($0.826$, \S\ref{sec:causal}) while the
identical band is catastrophic for FireRed ($0.035$, visually confirmed as
unrecoverable pixel noise in the car region, unlike \model{}'s coherent
survival). The readable/necessary dissociation of \S\ref{sec:swap} -- that
a layer band can be head-readable without being causally necessary -- is
therefore architecture-general only in its early half; whether the late
stack is safely redundant turns out to be a property of \emph{training},
not of the transformer block design. We read this as the cross-model
check doing exactly its job: catching a claim
(\S\ref{sec:causal}'s "L48--59 is dispensable for \emph{car\_red}") that,
stated without qualification, over-generalized past the one checkpoint it
was measured on.

\begin{figure*}[t]
  \centering
  \includegraphics[width=0.98\textwidth]{figs/fig16_crossmodel_lens.jpg}
...
bar) reproduces commitment-early: both cases' internal image already
...
\S\ref{sec:tunedlens}--\S\ref{sec:previewlever}'s three laws -- no hard
affine boundary, commitment-early-but-separation-gradual, and a usable
15\%-NFE preview -- are therefore properties of the shared transformer
architecture, not artifacts of one checkpoint's training, exactly like
the boundary and probe direction replicated above. The practical
...
from-scratch, architecturally distinct DiT is the stronger test of
external validity still owed.
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.98\textwidth]{figs/fig17_k7_ladder.jpg}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.98\textwidth]{figs/fig18_k8_decompose.jpg}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.98\textwidth]{figs/fig19_k9_curve.jpg}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.98\textwidth]{figs/h1a_family_probes.png}
...
\begin{figure}[!tp]
  \centering
  \includegraphics[width=0.88\columnwidth]{figs/h1b_steer_spectrum.jpg}
...
\textbf{The translation boundary is family-universal; decision depth is
not.} Wherever the A/C crossover is cleanly measurable it lands in the
same terminal band as color's L52.7--53.6: style at L53.0/53.1
($t{=}4$/$t{=}16$), background at L52.1 ($t{=}16$; at $t{=}4$ the
standard decode already wins from L48 on -- the swap has fully committed
by then). For removal and addition the between-variant CLIP margins are
...
anywhere else in the stack. Decision \emph{readability}, by contrast,
...
Combined with \S\ref{sec:when}'s
onset finding -- insertion edits are also the ones with delayed onset in
\emph{time} -- this is a two-axis regularity: appearance edits are
decided early in depth and early in time; structure edits crystallize
late in depth and, for insertion, late in time. One honest footnote: the
...
cleanly (at the depth where its decision has crystallized), a global
...
\begin{figure}[t]
  ...
  \begin{subfigure}[b]{0.47\columnwidth}
    \centering
    \includegraphics[width=\linewidth]{figs/i1d_recompose_grid.jpg}
    \caption{}
    \label{fig:recompose}
  \end{subfigure}
  \hfill
  \begin{subfigure}[b]{0.47\columnwidth}
    \centering
    \includegraphics[width=\linewidth]{figs/i2_taxonomy_grid.jpg}
    \caption{}
    \label{fig:taxonomy}
  \end{subfigure}
...
at L36 and applied at the deep L24--47 band where addition's decision
...
clean; L0--23 only a faint, uncommitted ghost) -- a third confirmation,
...
and unconditional -- at every one of the 20 steps; if the edit decision
is linearly present by layer 6 and translated by layer $\approx$53
(\S\ref{sec:mechanism}), there is no architectural reason CFG needs both
passes for all 20 steps. We test this by running the unconditional pass
...
\textbf{Dropping CFG to zero isolates its real job: scope, not
commitment.} At \texttt{cfg\_until=0} (zero unconditional forwards, a
measured $2.0\times$ speedup: $9.7$s vs.\ $19.3$s) recolor, removal, and
addition all still land -- the conditional pass alone commits and
renders these three, addition coming out smaller but present. Background
...
CFG-free pass synthesizes without erasing). CFG's non-redundant
contribution is thus not committing the edit -- the conditional pass
already does that -- but resolving \emph{scope}: deciding which
un-mentioned source content belongs to ``the background'' and must be
suppressed. A twenty-case battery (the full commit-time battery of
...
$1.77\times$), with per-family pixel deviation ordered exactly as the
commit-time law predicts (style 7.8--18.0, background 6.0--13.9,
...
sampling \emph{trajectory} but not the decision \emph{structure}: the
commit/onset machinery survives the $20\!\to\!4$ step compression.
...
This tool is exploratory. It is not a validated, peer-reviewed
interpretability method, it makes no claim about "intent" or
"understanding" in \model{} in any human sense, and every number in this
paper comes primarily from one model, mostly one or two seeds, and a
double-digit count of hand-chosen source images and prompts.
...
architecture. The edit-family sweep (\S\ref{sec:families}) likewise
...
\paragraph{Ablation semantics are blunt.} Our zero-ablation
...
answers "does the model recover if this specific position's information
is destroyed from this point on," not "does the model use a specific,
surgical direction within that representation." A layer band scoring
high edit-success under ablation means the network can route around the
loss of that information using what remains elsewhere at that
timestep -- it does not mean that layer band does nothing.
...
matching's velocity target is computed. \S\ref{sec:generalize} shows the
boundary location, the readable-decision layer, and the decision
direction replicate across two further checkpoints in \model{}'s own
lineage, but we do not claim this beyond that lineage, and
\S\ref{sec:generalize}'s own late-band divergence is a direct
counterexample to assuming any single causal finding transfers by
default. We do claim that, on \model{}, the specific
readable/necessary/sufficient dissociations we report are real and
repeatable given the seeds and prompts we used, because we checked them
against causal interventions rather than resting on passive readouts
alone.

\section{Conclusion}\label{sec:conclusion}

We built a layer--time lens for a 60-block DiT image editor by decoding
intermediate residual streams through the model's own frozen output
head, and paired it with linear probes and causal interventions
(ablation, hidden-state transplant) so that every passive reading has a
corresponding active check. The picture that emerges is a two-code
process: for most of the network's depth, the residual stream carries a
target-image code that a linear probe can already read out (as early as
layer 6) well before the network's own output head is willing to commit
to it, and only in the final $\sim$6 of 60 blocks is this translated
into the velocity code the sampler actually consumes. A denser sweep
pins this translation to a sharp, timestep-invariant band centered at
layer 53, and a parallel text-stream ablation locates the instruction's
\emph{injection} itself -- load-bearing in the first 11 blocks,
functionally complete by block 36 -- turning the two-code split into a
five-stage circuit: injection, decision, target-image code, translation,
output head. Reading and verifying are different operations with
different answers -- the layers where an edit is most easily \emph{read}
are not always the layers it causally \emph{depends on}, and the layers
sufficient to transplant an edit into a different run are not always the
layers necessary to produce it in the first place. Replicating the core
measurements on an independently fine-tuned checkpoint and a 4-step
distilled merge shows the boundary, the readable-decision layer, and the
decision direction itself transfer nearly unchanged, and a further
replication shows the entire tuned/differential lens toolchain transfers
the same way -- frozen-basis zero-shot, then closed by recalibration
rather than by refitting. Only one finding --
that the late stack is causally redundant -- turned out to be specific to
this checkpoint's training rather than to the architecture, which is
itself the kind of thing a cross-model check exists to catch. Extending
the same measurements across four further edit families kept the
boundary fixed but split the families along depth -- appearance edits
decided shallow, structure edits crystallizing only mid-stack -- and
steering each family's probe direction resolved what such a direction
actually carries: geometry-free semantics but, we thought, no shape --
while our own earlier reading of the color ``flip'' as a clean recolor
turned out to be a forced-choice metric hiding a flat patch. A follow-up
mask study forced a fourth correction: confining the identical direction
to the edited object's own silhouette turns every earlier flat-patch
failure into a perfect in-place recolor at any dose -- a direction
cannot carry \emph{where to stop}, not shape itself. Conjuring
an object, though, outruns any single direction: a rank ladder over the
donor$-$recipient difference resolves object identity into
coarse-to-fine linear shells (category at rank 1, a coherent instance
by rank 4, material by 16, donor-faithful by 64), with the probe
direction itself staying inert because the subspace a variable is read
from and the subspace that changes it are not the same. Linear
readability and linear writability thus dissociate the same way linear
readability and causal necessity do, depending on whether a variable is
an attribute of something already there or the identity of something
not yet there. A
third, donor-free candidate for that write basis -- the local Jacobian
toward a donor target -- turns out to overlap with neither: it writes
large but non-semantic content, the adversarial-gradient lesson
replayed inside a generative model's own latent space. A
parallel study found classifier-free guidance itself is largely
redundant after the first two steps, a training-free $45\%$ saving whose
real job is scope resolution rather than committing the edit -- a second
practical lever alongside \S\ref{sec:applications}'s three. Likewise,
extending our onset-timing measurement from one case to twenty replaced a
tidy continuous story we were initially tempted to tell with a narrower
but honest categorical one. If there is one sentence to take from this
paper, it is the one that organized our own thinking while writing it:
\emph{the lens shows the translation, not the decision -- reading the
decision requires changing basis, and verifying it requires
intervention.} Behind that methodological sentence stands the picture
it uncovered, the one this paper opened with and has now earned clause
by clause: an edit is not developed gradually -- it is \emph{decided
early}, \emph{bound to a spatial carrier} that determines whether it
can be rendered cleanly, and \emph{translated late} into the strokes
that paint it. That internal image is not only inferred from probes
and interventions -- a tuned lens renders it directly at every depth
(\S\ref{sec:tunedlens}), letting us watch the edit enter it layer by
layer at the very first step and exposing the mid-stack anti-image as
the same code, read through the wrong basis. A generative editor, in
other words, natively thinks
with an internal image before it makes one. The lens turns that thought
-- including its struggles, the late start of genuinely novel structure
and the recruitment of surrounding tokens when a direction lacks a
carrier -- into something that can be watched, steered, and budgeted
for. Differencing that same lens against a matched no-op run turns the
internal edit plan into a directly measurable quantity -- commitment is
early but separation into the edit region is gradual, not a step --
and, because the no-op run does not depend on which edit is asked for,
the same differential previews where a candidate edit will land after
only 15\% of the denoising trajectory
(\S\ref{sec:difflens}--\S\ref{sec:previewlever}).
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{figs/fig8_appendix.pdf}
