...
\begin{figure*}[t]
  ...
  head-readable signal first appears, but a linear probe shows the
  instruction is already linearly decodable from layer 6
  (\S\ref{sec:mechanism}), while fixed-prompt generation-level probes
  later establish early prediction of the realized outcome. A
  ...
  this as five measured stages (right): the instruction
  is injected early, the realized outcome becomes predictable, most of
  the stack carries a target-image code, a narrow band (L52--54)
  translates it into the velocity code, and layer 59's frozen head reads
  it out. Predictability and frozen-head readability are distinct.}
  \label{fig:teaser}
\end{figure*}
